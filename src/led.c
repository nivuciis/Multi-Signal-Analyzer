 ...
 *  Fourth state machine of 3nd pio block
 */
...
{
	gpio_set_function(LED_GPIO_PIN, GPIO_FUNC_SIO);
	return;
}

static void set_led_pin_to_pio(void)
{
	pio_gpio_init(pio_instance, LED_GPIO_PIN);
	return;
}
...
void ana_led_init(void)
{
	...
	blink_gated_program_init(pio_instance, sm, offset, LED_GPIO_PIN, LED_BLINK_PIN);
	gpio_set_function(LED_BLINK_PIN, GPIO_FUNC_SIO);
...
static const led_cmd led_op[_LED_STATUS_AMOUNT] = {[LED_STATUS_OFF] = {led_reset},
